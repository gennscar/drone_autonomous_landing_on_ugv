...
class UwbPubSub(Node):
    def __init__(self):
        ...
        self.anchor_0_subscriber = self.create_subscription(
            Odometry, "/anchor_0/odom", self.callback_anchor_0, 10)
        self.anchor_1_subscriber = self.create_subscription(
            Odometry, "/anchor_1/odom", self.callback_anchor_1, 10)
        self.anchor_2_subscriber = self.create_subscription(
            Odometry, "/anchor_2/odom", self.callback_anchor_2, 10)
        self.anchor_3_subscriber = self.create_subscription(
            Odometry, "/anchor_3/odom", self.callback_anchor_3, 10)
        self.chassis_subscriber = self.create_subscription(
            Odometry, "/chassis/odom", self.callback_chassis, 10)

        self.target_coordinates_publisher = self.create_publisher(
            Pose, "target_coordinates", 10)
    ...
    def timer_callback(self):
        ...
        if STD_TRILATERATION == True:
            self.y = standard_trilateration.trilateration(
                self.d0, self.d1, self.d2, self.d3)
        else:
            self.y = gauss_newton_trilateration.trilateration(
                self.y, self.d0, self.d1, self.d2, self.d3)

        self.rel_pos_anchor0 = self.y[1:]
        self.rot_world_to_anchor0 = R.from_quat(self.anchor_0_orientation)

        self.rel_pos_chassis = np.subtract(
            self.rel_pos_anchor0, chassis_wrt_anchor0)
        self.rel_pos_world_frame = - \
            self.rot_world_to_anchor0.apply(self.rel_pos_chassis)
